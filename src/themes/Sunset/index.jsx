...
const SunsetTheme = ({ config }) => {
  ...
  return (
    <div className={`relative min-h-screen bg-gradient-to-br from-[#ff7e5f] to-[#feb47b] text-[#F5F5F7] font-sans selection:bg-yellow-300 selection:text-black ${appearance?.backgroundColor || ''} ${appearance?.primaryTextColor || ''}`}>
      ...
      <section className="relative z-10 min-h-[90vh] flex flex-col items-center justify-center px-6 overflow-hidden">
        {/* Subtle background glow (The Sun) */}
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-40 mix-blend-screen">
          <div className="w-[80vw] h-[80vw] max-w-[800px] max-h-[800px] bg-yellow-400 rounded-full blur-[120px]" />
        </div>
        ...
        >
          {recipient.introduction && (
            <motion.p 
              ...
              className="text-white/80 text-lg md:text-xl font-medium tracking-widest uppercase mb-6"
            >
              {recipient.introduction}
            </motion.p>
          )}
          ...
          {content.subheading && (
            <motion.p 
              ...
              className="text-xl md:text-3xl text-white/90 font-medium max-w-2xl mx-auto leading-relaxed drop-shadow-sm"
            >
              {content.subheading}
            </motion.p>
          )}
      ...
      {/* Message Section */}
      <section className="py-32 px-6 bg-black/30 backdrop-blur-md border-y border-white/10">
        <motion.div 
          initial="hidden"
          whileInView="visible"
        ...
        >
          <p 
            ...
            className={`font-serif text-3xl md:text-5xl lg:text-6xl leading-tight text-white font-medium drop-shadow-md ${customStyles.paragraph || ''}`}>
            "{content.message}"
          </p>
        </motion.div>
      </section>
        ...
        >
          ...
          {features.showButton && content.buttonText && (
            <motion.div variants={fadeUp} className="pt-8">
              <ScratchCard enabled={features.enableScratchCard} text={features.scratchCardText} coverColor="#d97706">
              <a 
                href={content.buttonLink || '#'} 
                style={{ color: content.buttonTextColor || undefined }} 
                className={`inline-block px-10 py-4 bg-white text-rose-600 rounded-full font-semibold text-lg hover:scale-105 hover:bg-yellow-50 hover:text-orange-500 shadow-xl transition-all duration-300 ${customStyles.button || ''}`}
              >
                {content.buttonText}
              </a>
              </ScratchCard>
